...
int msm_q6_enable_mi2s_clocks(bool enable) 
{
	...
	if (enable) {
		...
		port_config.i2s.mono_stereo = MSM_AFE_STEREO;
...
static int msm_quat_mi2s_clock_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s\n", __func__);
	return 0;
...
static int msm_quat_mi2s_clock_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int ret = -EINVAL;
	struct msm8916_asoc_mach_data *pdata = NULL;
	...
	pr_debug("%s: quat mclk enable %ld\n",
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("QUAT_MI2S Clock", msm_snd_enum[2],
			msm_quat_mi2s_clock_get, msm_quat_mi2s_clock_put),
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	pinctrl_info.cdc_lines_dmic_sus = pinctrl_lookup_state(pinctrl_info.pinctrl,
		"cdc_lines_dmic_sus");
	if (IS_ERR(pinctrl_info.cdc_lines_dmic_sus)) {
		pr_err("%s: Unable to get pinctrl cdc_lines_dmic_sus handle\n",
							__func__);
	}
	pinctrl_info.cdc_lines_dmic_act = pinctrl_lookup_state(pinctrl_info.pinctrl,
		"cdc_lines_dmic_act");
...
static void msm_quat_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if ((pdata->ext_pa & QUAT_MI2S_ID) == QUAT_MI2S_ID) {
		...
		pr_err("%s Quaternary MI2S Clock is Disabled", __func__);
...
static int msm8952_asoc_machine_probe(struct platform_device *pdev)
{
	...
	pinctrl_info.pinctrl = devm_pinctrl_get(&pdev->dev);
