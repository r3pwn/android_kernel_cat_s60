...
/* CEI sam add to get/set resume flag */
#include "mdss_fb.h"
...
/* CEI sam add for SRE*/
static void mdss_dsi_panel_sre_ctrl(struct mdss_panel_data *pdata, u32 enable_sre);
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	/* CEI sam add for SRE */
	if (mdss_fb_get_resume_flag() >= 0 && mdss_fb_get_resume_flag() <= 3) {
		mdss_dsi_panel_sre_ctrl(pdata, mdss_fb_get_resume_flag());
		mdss_fb_set_resume_flag(-1);
	}
end:
	pr_debug("%s:-\n", __func__);
	return ret;
...
/* CEI sam add for sunlight readable enhancement */
static char sre_disable[2] = {0x55, 0x00};
static char sre_low_enable[2] = {0x55, 0x40};
static char sre_middle_enable[2] = {0x55, 0x50};
static char sre_high_enable[2] = {0x55, 0x60};

static char read_id[2] = {0x04, 0x00};
static int read_id_len = 3;

static char read_cabc[2] = {0x56, 0x00};
static int read_cabc_len = 1;

static struct dsi_cmd_desc sre_disable_cmd = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(sre_disable)},
	sre_disable
};

static struct dsi_cmd_desc sre_low_enable_cmd = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(sre_low_enable)},
	sre_low_enable
};

static struct dsi_cmd_desc sre_middle_enable_cmd = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(sre_middle_enable)},
	sre_middle_enable
};

static struct dsi_cmd_desc sre_high_enable_cmd = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(sre_high_enable)},
	sre_high_enable
};
...
static void mdss_dsi_panel_sre_ctrl(struct mdss_panel_data *pdata, u32 enable_sre)
{
	...
	memset(&cmdreq, 0, sizeof(cmdreq));
	if (enable_sre == 0)
		cmdreq.cmds = &sre_disable_cmd;
	else if (enable_sre == 1)
		cmdreq.cmds = &sre_low_enable_cmd;
	else if (enable_sre == 2)
		cmdreq.cmds = &sre_middle_enable_cmd;
	else if (enable_sre == 3)
		cmdreq.cmds = &sre_high_enable_cmd;
...
/* CEI sam add to read panel id */
static void mdss_dsi_panel_get_id(struct mdss_panel_data *pdata, char *rx_buf)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	char rx_id[10];
	u32 i, len = 0;
	...
	mdss_dsi_panel_cmd_read(ctrl_pdata, read_id[0], read_id[1],
				NULL, rx_id, read_id_len);

	for (i = 0; (len < 128) && (i < ctrl_pdata->rx_len); i++)
		len += scnprintf(rx_buf + len, 128 - len,
				"%02x", rx_id[i]);

	     rx_buf[len] = '\0';
}

/* CEI sam add to read cabc */
static void mdss_dsi_panel_get_cabc(struct mdss_panel_data *pdata, char *rx_buf)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	char rx_cabc[10];
	u32 i, len = 0;
	...
	mdss_dsi_panel_cmd_read(ctrl_pdata, read_cabc[0], read_cabc[1],
				NULL, rx_cabc, read_cabc_len);
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	/* CEI sam add for sunlight readable enhancement */
	ctrl_pdata->panel_data.set_sunlightreadable = mdss_dsi_panel_sre_ctrl;
	/* CEI sam add for read panel id */
	ctrl_pdata->panel_data.get_panel_id = mdss_dsi_panel_get_id;
	/* CEI sam add to get cabc */
	ctrl_pdata->panel_data.get_cabc = mdss_dsi_panel_get_cabc;
