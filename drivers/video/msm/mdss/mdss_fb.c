...
/* CEI sam add for sunlight readable enhancement */
int resume_flag = -1;
static struct led_classdev sre_led = {
	.name           = "lcd-sre",
	.brightness     = 0,
	.brightness_set = mdss_fb_set_sunlightreadable,
	.max_brightness = 32,
};

/* CEI sam add to store current bl_scale */
u32 current_bl_scale = 1024;
...
/* CEI sam add to get panel id */
static ssize_t mdss_fb_get_display_id(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	char rx_buf[128];
	...
	pdata = dev_get_platdata(&mfd->pdev->dev);
	if ((pdata) && (pdata->get_panel_id)) {
		pdata->get_panel_id(pdata, rx_buf);
		ret = snprintf(buf, 128, "%s\n", rx_buf);
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	pdata = dev_get_platdata(&mfd->pdev->dev);
	if ((pdata) && (pdata->get_cabc)) {
		pdata->get_cabc(pdata, rx_buf);
		ret = snprintf(buf, 128, "%s\n", rx_buf);
	}
...
/* CEI sam add device attribute to read driver ic registers */
static DEVICE_ATTR(display_id, S_IRUGO, mdss_fb_get_display_id, NULL);
/* CEI sam add device attribute to read cabc register */
static DEVICE_ATTR(cabc, S_IRUGO, mdss_fb_get_cabc, NULL);
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	/* CEI sam add for sunlight readable enhancement */
	if (!lcd_sre_registered) {
		if (led_classdev_register(&pdev->dev, &sre_led))
			pr_err("led_classdev_register failed\n");
		else
			lcd_sre_registered = 1;
	}
...
static int mdss_fb_remove(struct platform_device *pdev)
{
	...
	/* CEI sam add for sunlight readable enhancement */
	if (lcd_sre_registered) {
		lcd_sre_registered = 0;
		led_classdev_unregister(&sre_led);
	}
...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		if (mfd->bl_level_scaled == temp) {
			...
			/* CEI sam add for Bug 1416, do not sent pp_bl_event when bl_scale does not change */
			if (current_bl_scale == mfd->bl_scale)
				bl_notify_needed = false;
			else
				current_bl_scale = mfd->bl_scale;
		}
...
/* CEI sam add for sunlight readable enhancement */
void mdss_fb_set_sunlightreadable(struct led_classdev *led_cdev, u32 enable_sre)
{
	struct msm_fb_data_type *mfd;
	struct mdss_panel_data *pdata;

	mfd = dev_get_drvdata(led_cdev->dev->parent);
	pdata = dev_get_platdata(&mfd->pdev->dev);


	if (mfd->panel_power_state) {
		pdata->set_sunlightreadable(pdata, enable_sre);
		return;
	}
	/* delay the command until Resume because power state is OFF */
	else
		resume_flag = enable_sre;
}
...
/* CEI sam add to get/set resume flag to delay to send SRE command */
int mdss_fb_get_resume_flag(void)
{
	return resume_flag;
}
EXPORT_SYMBOL(mdss_fb_get_resume_flag);

void mdss_fb_set_resume_flag(int flag)
{
	resume_flag = flag;
}
EXPORT_SYMBOL(mdss_fb_set_resume_flag);
