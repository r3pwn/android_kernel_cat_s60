...
static int smb_alarm_period = 6*5;
struct smbchg_chip *smb_updata_chip;
struct alarm smb_alarm_suspend;
struct timer_list smb_timer;
struct work_struct smb_updata;
struct work_struct smb_updata_suspend;
#define TRIM_PERIOD_NS (10LL * NSEC_PER_SEC)
...
void smbchg_chg_rled_brightness_set(struct smbchg_chip *chip,
		enum led_brightness value)
{
	u8 reg;
	int rc;

	dev_err(chip->dev, "smbchg_chg_rled_brightness_set\n");
...
static void smb_updata_work(struct work_struct *work)
{
	power_supply_changed(&smb_updata_chip->batt_psy);
	pr_debug("smb_updata_work\n");
	smb_timer.expires = jiffies + 60*HZ;
	add_timer(&smb_timer);

	pm_relax(smb_updata_chip->dev);

}

static void smb_updata_suspend_work(struct work_struct *work)
{
	ktime_t kt;
	int temp;

	temp = get_prop_batt_temp(smb_updata_chip);
	pr_err("smb_updata_suspend_work\n");
	pr_err("get_prop_batt_temp= %d\n", temp);
	if ((temp > 530) || (temp < 20)) {
		smb_alarm_period = 3;
		power_supply_changed(&smb_updata_chip->batt_psy);
	} else{
		smb_alarm_period = 6*10;
	}

	kt = ns_to_ktime(TRIM_PERIOD_NS*smb_alarm_period);
	alarm_start_relative(&smb_alarm_suspend, kt);

	pm_relax(smb_updata_chip->dev);
}


static enum alarmtimer_restart smb_report_suspend(struct alarm *alarm,
					ktime_t now)
{
	pr_debug("smb_report_suspend\n");
	schedule_work(&smb_updata_suspend);
	pm_stay_awake(smb_updata_chip->dev);
	pr_debug("smb_report_suspend schedule_work OK\n");

	return ALARMTIMER_NORESTART;
}

static void smb_report(unsigned long data)
{
	pr_debug("smb_report\n");
	schedule_work(&smb_updata);
	pm_stay_awake(smb_updata_chip->dev);
	pr_debug("smb_report schedule_work OK\n");
}


static void smb_init_timer(void)
{
	ktime_t kt;

	pr_err("smb_timer_init start\n");
	alarm_init(&smb_alarm_suspend, ALARM_REALTIME, smb_report_suspend);
	init_timer(&smb_timer);
	smb_timer.function = &smb_report;
	smb_timer.data = ((unsigned long) 0);
	smb_timer.expires = jiffies + 10*HZ;
	add_timer(&smb_timer);
	kt = ns_to_ktime(TRIM_PERIOD_NS*6*5);
	alarm_start_relative(&smb_alarm_suspend, kt);
}

struct smbchg_chip *tchip;
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	INIT_WORK(&smb_updata, smb_updata_work);
	INIT_WORK(&smb_updata_suspend, smb_updata_suspend_work);
