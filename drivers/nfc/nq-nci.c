    ...
    */
    ...
    if (MODE_RUN == mode) {
        ...
        if (dev->spi_ven_enabled == false) {
            gpio_set_value(dev->ven_gpio, 0); //CEI porting
            msleep(100);
        }
    }
        ...
        if (dev->spi_ven_enabled == false)
        {
            ...
            msleep(20);
            gpio_set_value(dev->ven_gpio, 1); //CEI porting
            msleep(100);
            gpio_set_value(dev->ven_gpio, 0); //CEI porting
            msleep(20);
        }
                           ...
                           struct pn5xx_i2c_platform_data *pdata)
{
    ...
    val = of_get_named_gpio(node, "qcom,nq-ven", 0);
    if (val >= 0) {
        pdata->ven_gpio = val;
    }
    else{
    ...
    val = of_get_named_gpio(node, "qcom,nq-firm", 0);
    if (val >= 0) {
        pdata->firm_gpio = val;
    }
    else {
    ...
    val = of_get_named_gpio(node, "qcom,nq-irq", 0);
    if (val >= 0) {
        pdata->irq_gpio = val;
    }
    else {
    ...
    val = of_get_named_gpio(node, "qcom,nq-clkreq", 0);
    if (val >= 0) {
...
#endif
{
    ...
    if (gpio_is_valid(pdata->clkreq_gpio)) {
        pr_err("%s: request clkreq_gpio %d\n", __func__, pdata->clkreq_gpio);
