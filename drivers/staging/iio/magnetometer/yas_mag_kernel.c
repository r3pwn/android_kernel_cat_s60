...
static ssize_t yas_self_test_all_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct yas_state *st = iio_priv(indio_dev);
...
static ssize_t yas_self_ping_show(struct device *dev,
	       struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct yas_state *st = iio_priv(indio_dev);
...
static void yas_work_func(struct work_struct *work)
{
	...
	if (ret == 1 && first_measure == 0)
		yas_data_rdy_trig_poll(indio_dev);

	if (first_measure) {
		first_measure = 0;
		pr_debug("yas_work_func: skip\n");
	}
...
static IIO_DEVICE_ATTR(self_test_all, S_IRUSR, yas_self_test_all_show, NULL, 0);
...
static IIO_DEVICE_ATTR(ping, S_IRUSR, yas_self_ping_show, NULL, 0);
...
static struct attribute *yas_attributes[] = {
	...
	&iio_dev_attr_self_test_all.dev_attr.attr,
...
static int yas_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (hwid > DVT1) {
		pr_err("yas driver not for DVT1\n");
		return -EFAULT;
	}
	this_client = i2c;
	...
	ret = yas_mag_driver_init_dvt1(&st->mag);
	if (ret < 0) {
		ret = -EFAULT;
...
static struct of_device_id yas537_dts_table[] = {
	{
...
static struct i2c_driver yas_driver = {
	.driver = {
		...
		.owner	= THIS_MODULE,
		.pm	= YAS_PM_OPS,
		.of_match_table = yas537_dts_table,
	},
	.probe		= yas_probe,
	.remove		= yas_remove,
	.id_table	= yas_id,
