...
#define MIN_ALS_POLL_DELAY_NS	60000000
	...
	#define STK_HT_N_CT	250
        //STK add for CCI start 20150318
        unsigned int cci_ps_high_thd;
        unsigned int cci_ps_low_thd;

        //add for FTM interrupt check 20150326 start
          int CCI_FTM_INTERRUPT = 0;
        //add for FTM interrupt check 20150326 end
...
unsigned int cci_als_value_test;
unsigned int cci_transmittance_cali;
unsigned int cci_als_adc_cali;
//20150318

int dummyLuxValue = -1;
//--20150318 configuration start
...
static struct kobject *stk3x1x_als_sysfs_link;
static struct kobject *stk3x1x_ps_sysfs_link;
/*
extern void elan_call_suspend(void);//20150806
extern void elan_call_resume(void);//20150806
extern void elan_call_ic_reset(void);//20151021
*/
/*
#define Himax_touch
#ifdef Himax_touch
extern int P_sensor_call_TP_suspend(void);
extern int P_sensor_call_TP_resume(void);
extern void P_sensor_call_TP_reset(void);
#endif
 */
...
static uint32_t stk3x1x_get_ps_reading(struct stk3x1x_data *ps_data)
{
	...
	if(err < 0)
	{
		printk(KERN_ERR "%s: stk3x1x_get_ps_reading fail, ret=%d\n", __func__, err);
		return err;
	}
...
static int32_t stk3x1x_get_als_reading(struct stk3x1x_data *ps_data)
{
	...
	{
		if(als_data < 30 && ps_data->als_code_last > 10000) //20150513 autoBrightness
		{
			...
			if(ir_data > 400) //20150513 autoBrightness
			{
...
//--20150318 add GC_3cm node start

 static uint32_t stk3x1x_get_ps_reading_AVG(int sSampleNo,struct stk3x1x_data *ps_data){
          uint32_t PSData = 0;
...
static ssize_t stk3x1x_show_GC_ht(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	uint32_t reading;
	int err;
	//PS cal Gray card 2 cm
	if (!ps_data) {
		APS_ERR("[#23]stk3x1x_obj is null!!\n");
		return 0;
	}
	//set default THD as ps_thd_h = 0, ps_thd_l = 0 to ensure int trigger start
	ps_data->ps_thd_h = 0;
	ps_data->ps_thd_l = 0;
	APS_ERR("[#23]GC_ht: ps_thd_h = %d, ps_thd_l = %d\n", ps_data->ps_thd_h, ps_data->ps_thd_l);
	...
	//set default THD as ps_thd_h = 0, ps_thd_l = 0 to ensure int trigger end
	APS_ERR("[#23]%s: Start calibrating with Gary Card high threshold...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5, ps_data);
	cci_ps_high_thd = reading;//*****************This is F2N threshold***************
	APS_ERR("[#23]%s: Calibrating with Gray Card high threshold done!!!!! cci_ps_high_thd = %d\n", __func__, cci_ps_high_thd);
	return scnprintf(buf, PAGE_SIZE, "PASS: cci_ps_high_thd = %d\n", cci_ps_high_thd);
}

static ssize_t stk3x1x_show_GC_lt(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	uint32_t reading;
	//PS cal Gray card low threshold
	if (!ps_data) {
		APS_ERR("[#23]stk3x1x_obj is null!!\n");
		return 0;
	}
	APS_ERR("[#23]%s: Start calibrating with Gary Card low threshold...\n", __func__);
	msleep(150);
	reading = stk3x1x_get_ps_reading_AVG(5, ps_data);
	cci_ps_low_thd = reading;
	APS_ERR("[#23]%s: Calibrating with Gray Card low threshold done!!!!! cci_ps_low_thd = %d\n", __func__, cci_ps_low_thd);
	return scnprintf(buf, PAGE_SIZE, "%s: cci_ps_low_thd = %d\n",
			cci_ps_high_thd > cci_ps_low_thd ? "PASS" : "FAIL", cci_ps_low_thd);
}
//--20150402 add GC_3cm node end

//--20150402 add GC_ct node start
static ssize_t stk3x1x_show_GC_ct(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	...
	if (cci_result_ct < 1800) {
		//cci_ps_high_thd = cci_ps_low_thd + 190;
		if (cci_ps_high_thd > cci_ps_low_thd
				&& cci_ps_low_thd > cci_result_ct) {
			/*
			   if (cci_ps_low_thd - cci_result_ct > 50
			   && cci_ps_high_thd - cci_ps_low_thd > 50) {
			   */
			...
			return scnprintf(buf, PAGE_SIZE, "FAIL: cci_result_ct = %d\n", cci_result_ct);
		}
	} else {
		APS_ERR("[#23]--------------- CT < 1000 fail----------------\n");
		APS_ERR("[#23] cci_result_ct = %d\n", cci_result_ct);
		return scnprintf(buf, PAGE_SIZE, "FAIL: cci_result_ct = %d\n", cci_result_ct);
	}
	APS_ERR("[#23] Calibration CT PASS: cci_ps_high_thd = %d, cci_ps_low_thd = %d, cci_result_ct = %d\n", cci_ps_high_thd, cci_ps_low_thd, cci_result_ct);
	return scnprintf(buf, PAGE_SIZE, "PASS: cci_result_ct = %d\n", cci_result_ct);
}
//--20150402 add GC_ct node end

//--20150318 add test_ht node start
static ssize_t stk3x1x_show_GC_test(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	uint32_t reading;
	...
	APS_ERR("%s:[#23]Start testing GC_1cm ...\n", __func__);

	reading = stk3x1x_get_ps_reading_AVG(5, ps_data);
	test_ps_code = reading;
	//add for FTM interrupt check 20150326 start
	APS_ERR("%s:[#23]stk3x1x_show_GC_test() CCI_FTM_INTERRUPT = %d \n", __func__, CCI_FTM_INTERRUPT);

	if (test_ps_code >= cci_ps_high_thd) {
		ps_test_cover = true;
		cci_ps_cover = test_ps_code;
		if (CCI_FTM_INTERRUPT != 0) {
			APS_ERR("[#23]Test PS Gray card 1cm PASS: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
			return scnprintf(buf, PAGE_SIZE, "PASS: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
		} else {
			ps_test_cover = false;
			APS_ERR("[#23]Test PS Gray card 1cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT FAIL\n", ps_test_cover, cci_ps_cover);
		...
		cci_ps_cover = test_ps_code;
		if (CCI_FTM_INTERRUPT != 0) {
			APS_ERR("[#23]Test PS Gray card 1cm FAIL: ps_test_cover = %d, g_result_ps_cover = %d, INT PASS\n", ps_test_cover, cci_ps_cover);
...
//--20150318 add test_ht node end

//--20150318 add test_lux node start
static uint32_t stk3x1x_get_als_reading_AVG(int sSampleNo,struct stk3x1x_data *ps_data){
            uint32_t ALSData = 0;
            uint32_t DataCount = 0;
			...
			//w_state_reg = STK_STATE_EN_ALS_MASK;
			//ret = stk3x1x_set_state(ps_data, w_state_reg);
			//ret = stk3x1x_enable_als(ps_data, 1);
			//if(ret < 0)
			//	return ret;
            while(DataCount < sSampleNo)
            {
					...
					//{
						...
						if(ret < 0)
						{
								APS_ERR("fail to get als code = %d \n", ret);
								return ret;
						}
						ALSData = (value[0]<<8) | value[1];
						APS_ERR("[#23]als code = %d \n", ALSData);
						SaveAlsData +=  ALSData;
						DataCount++;
					//}
            }
			...
			//w_state_reg = 0;
			//ret = stk3x1x_set_state(ps_data, w_state_reg);
			//ret = stk3x1x_enable_als(ps_data, 0);
			//if(ret < 0)
			//	return ret;
...
static ssize_t cci_dummy_lux_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "CCI dummy Lux value = %d\n", dummyLuxValue);
}
static ssize_t cci_dummy_lux_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int value = -1;
	int ret;
...
static ssize_t stk_als_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	int32_t als_reading;
	...
	msleep(150);
	als_reading = stk3x1x_get_als_reading_AVG(5, ps_data);
	mutex_lock(&ps_data->io_lock);
	APS_ERR("[#23]%s: als_reading = %d \n", __func__, als_reading);
	cci_als_value_test = stk_alscode2lux(ps_data, als_reading);
	mutex_unlock(&ps_data->io_lock);
	APS_ERR("[#23]%s: Start testing light done!!! cci_als_value_test = %d lux\n", __func__, cci_als_value_test);
	APS_ERR("[#23]%s: als_transmittance = %d, cci_transmittance_cali = %d \n",__func__,ps_data->als_transmittance, cci_transmittance_cali);
	if(cci_als_value_test < 255 || cci_als_value_test > 345)
	{
		return scnprintf(buf, PAGE_SIZE, "FAIL: cci_als_value_test = %d\n", cci_als_value_test);
	}
	return scnprintf(buf, PAGE_SIZE, "PASS: cci_als_value_test = %d\n", cci_als_value_test);

}

static ssize_t stk_als_light_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	int32_t als_reading;
	int err;
	if (!ps_data) {
		APS_ERR("ps_data is null!!\n");
		return 0;
	}
	APS_ERR("[#23]%s: Start Cali light...\n", __func__);
	msleep(150);
	als_reading = stk3x1x_get_als_reading_AVG(5, ps_data);
	...
	// 20150407 Set range for LT32 First
	if (cci_als_adc_cali <= 13 || cci_als_adc_cali >= 2100) {
		APS_ERR("[#23]stk3x1x_cci_als_cali() ALS out of range, als_reading = %d\n", cci_als_adc_cali);
		mutex_unlock(&ps_data->io_lock);
		return scnprintf(buf, PAGE_SIZE, "FAIL: cci_als_adc_cali = %d\n", cci_als_adc_cali);
	}
	//compute transmittance for ALS calibration start
	//if cci_als_value = 0, the cci_transmittance_cali will be always 0,so skip it.
	if (((cci_als_value * ps_data->als_transmittance) / 500) > 0) {
		cci_transmittance_cali = (cci_als_value * ps_data->als_transmittance) / 500; //transmittance for cali
		ps_data->als_transmittance = cci_transmittance_cali; // writ back to als_transmittance
		// Calculate new lux value by new transmittance, it'll close to 500
		cci_als_value = stk_alscode2lux(ps_data, als_reading);
		APS_ERR("[#23]%s: cci_transmittance_cali = %d, cci_als_adc_cali = %d, cci_als_value = %d lux\n", __func__, cci_transmittance_cali, cci_als_adc_cali, cci_als_value);
		err = 0;
	} else {
		APS_ERR("[#23]%s cali light fail!!! cci_als_value cannot be 0 \n", __func__);
		err = 1;
	}
	//compute transmittance for ALS calibration end
	mutex_unlock(&ps_data->io_lock);

	return scnprintf(buf, PAGE_SIZE, "%s: cci_transmittance_cali = %d, cci_als_adc_cali = %d code, cci_als_value = %d lux\n", err == 0 ? "PASS" : "FAIL", cci_transmittance_cali, cci_als_adc_cali, cci_als_value);
}

//--20150318 add test_lux node end
...
static ssize_t stk_ps_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/*if(en==0)
	{
		printk(KERN_ERR "%s: elan_call_ic_reset", __func__);
		//elan_call_resume();
		elan_call_ic_reset();
	}*/
    //======================
...
static ssize_t get_als_devnum(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	int devnum, ret;
	const char *devname = NULL;

	devname = dev_name(&ps_data->als_input_dev->dev);
	ret = sscanf(devname+5, "%d", &devnum);
	if (!ret)
		APS_ERR("[Sensor] get light sensor device number faild\n");

	return snprintf(buf, PAGE_SIZE, "%d\n", devnum);
}
static ssize_t get_ps_devnum(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data =  dev_get_drvdata(dev);
	int devnum, ret;
	const char *devname = NULL;

	devname = dev_name(&ps_data->ps_input_dev->dev);
	ret = sscanf(devname+5, "%d", &devnum);
	if (!ret)
		APS_ERR("[Sensor] get proximity sensor device number faild\n");
...
//--20150318 add L sensor calibration device node start 20150318
static struct device_attribute als_light_show = __ATTR(light_lux, 0664, stk_als_light_show, NULL);
static struct device_attribute als_test_show = __ATTR(test_lux, 0664, stk_als_test_show, NULL);
//--20150318 add L sensor calibration device node end 20150318
static struct device_attribute dummy_lux = __ATTR(dummy_lux, 0664, cci_dummy_lux_show, cci_dummy_lux_store);
#ifdef STK_ALS_FIR
static struct device_attribute als_firlen_attribute = __ATTR(firlen,0664,stk_als_firlen_show,stk_als_firlen_store);
#endif
static struct device_attribute als_devnum_attribute = __ATTR(devnum, 0664, get_als_devnum, NULL);
...
//add P sensor calibration device node start 20150318
static struct device_attribute ps_GC_ht_show = __ATTR(GC_ht, 0664, stk3x1x_show_GC_ht, NULL);
static struct device_attribute ps_GC_lt_show = __ATTR(GC_lt, 0664, stk3x1x_show_GC_lt, NULL);
static struct device_attribute ps_GC_ct_show = __ATTR(GC_ct, 0664, stk3x1x_show_GC_ct, NULL);
static struct device_attribute ps_GC_test_show = __ATTR(GC_test, 0664, stk3x1x_show_GC_test, NULL);
//add P sensor calibration device node end 20150318
#ifdef STK_TUNE0
static struct device_attribute ps_cali_attribute = __ATTR(cali,0664,stk_ps_cali_show,stk_ps_cali_store);
//CCI  add start 20150330
static struct device_attribute ps_nvdata_attribute = __ATTR(nvdata,0664,stk_ps_show_nvdata, NULL);
//CCI  add end 20150330
...
#endif
static struct device_attribute ps_devnum_attribute = __ATTR(devnum, 0664, get_ps_devnum, NULL);
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
	...
	diff = ps_data->psa - ps_data->psi;//diff > 200
	if(diff > ps_data->stk_max_min_diff)
	{
		ps_data->psi_set = ps_data->psi;
		//20150720 start
		if(cci_ps_high_thd == 0 || cci_ps_low_thd == 0)
		{
		ps_data->ps_thd_h = ps_data->psi_set + ps_data->stk_ht_n_ct;
		ps_data->ps_thd_l = ps_data->psi_set + ps_data->stk_lt_n_ct;
		}
		else
		{
		...
		//20150720 end
                //20150513 To avoid Auto Calibration CT is too big
#ifdef CALI_PS_EVERY_TIME
                if(ps_data->ps_thd_h > 2500 && cci_ps_high_thd !=0 )
                {
                   printk(KERN_INFO "[stk] use factory CCI calibration value\n");
                   ps_data->ps_thd_h = cci_ps_high_thd+300;
                   ps_data->ps_thd_l = cci_ps_low_thd+300;
                   printk(KERN_INFO "%s: pocket mode HT=%d, LT=%d\n", __func__, ps_data->ps_thd_h, ps_data->ps_thd_l);
		}
		else if(ps_data->ps_thd_h > ps_data->ps_high_thd_boot)
		{
			...
			printk(KERN_INFO "%s: update boot HT=%d, LT=%d\n", __func__, ps_data->ps_high_thd_boot, ps_data->ps_low_thd_boot);
                ...
                printk(KERN_INFO "%s: set value: HT = %d, LT = %d\n",__func__,ps_data->ps_thd_h, ps_data->ps_thd_l);
#endif
                //20150513
		stk3x1x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
		stk3x1x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
...
//#ifdef CONFIG_HAS_EARLYSUSPEND
	...
	stk3x1x_als_sysfs_link = kobject_create_and_add("als", NULL);
	if (stk3x1x_als_sysfs_link != NULL)
		err = sysfs_create_link(stk3x1x_als_sysfs_link, &ps_data->als_input_dev->dev.kobj, "link");
	else
		err = -ENODEV;

	if (err < 0) {
		APS_ERR("%s: could not create sysfs link for stk3x1x light sensor\n", __func__);
		//goto err_setup_input_device;
	}
	stk3x1x_ps_sysfs_link = kobject_create_and_add("ps", NULL);
	if (stk3x1x_ps_sysfs_link != NULL)
		err = sysfs_create_link(stk3x1x_ps_sysfs_link, &ps_data->ps_input_dev->dev.kobj, "link");
	else
		err = -ENODEV;

	if (err < 0) {
		APS_ERR("%s: could not create sysfs link for stk3x1x proximity sensor\n", __func__);
		//goto err_setup_input_device;
	}
