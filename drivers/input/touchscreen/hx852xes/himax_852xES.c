...
#define GLOVE_FINGER_MODE
/* #define REGULATOR_AUTO_FOR_SYSTEM */
...
static u8 g_proximity_en = 0;
	...
	//#include "CEI_BT52_CAT_E18_C00_2016_01_12.i"
	//#include "CEI_BT52_CAT_E18_C00_2016_01_12_Frist_TMP.i"
	//#include "CEI_BT52_CAT_E1902_C01.i"
	//#include "CEI_BT52_CAT_E1904_C03.i"
	//#include "CEI_BT52_CAT_E1904_C04.i" */
	static unsigned char i_CTPM_FW[]=
	{
		#include "CEI_BT52_CAT_E1906_C06_2016-11-18.i"
	};
//for BIEL keep FW
	static unsigned char i_CTPM_FW2[]={
		#include "CEI_BT52_CAT_E1904_C04.i"
	};
...
static uint8_t  HX_DRIVER_PROBE_Fial = 0x00;
static int	hx_point_num	= 0x00; /* for himax_ts_work_func use */
...
static struct regulator *vdd = 0x00; /* 20160118 */
static int iref_number = 11;
static bool iref_found = false;    
static int once_run = 1; /* for CEI FACTORY FLAG */
static long LCM_ID = 0x00; /* for CEI FACTORY FLAG */
#ifdef GLOVE_FINGER_MODE
static int glove = 0X02;
#endif
...
static char android_boot_dev_display[10];
static int get_android_boot_dev_display(char *str)
{
	strlcpy(android_boot_dev_display, str, 10);
	return 1;
}
__setup("androidboot.displayid=", get_android_boot_dev_display);
...
static int i_update_FW(void)
{
	...
	if(LCM_ID == 1){
		...
		IUF = (( private_ts->vendor_fw_ver_H != i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] )
		...
		I("IMAGE FW_VER=%x,%x.\n",i_CTPM_FW2[FW_VER_MAJ_FLASH_ADDR],i_CTPM_FW2[FW_VER_MIN_FLASH_ADDR]);
		I("IMAGE CFG_VER=%x.\n",i_CTPM_FW2[FW_CFG_VER_FLASH_ADDR]);
		D("BIEL\n");

		IUF = (( private_ts->vendor_fw_ver_H != i_CTPM_FW2[FW_VER_MAJ_FLASH_ADDR] )
			|| ( private_ts->vendor_fw_ver_L != i_CTPM_FW2[FW_VER_MIN_FLASH_ADDR] ) 
			|| ( private_ts->vendor_config_ver != i_CTPM_FW2[FW_CFG_VER_FLASH_ADDR] )
			|| ( himax_calculateChecksum(false) == 0 ));
			...
			if(fts_ctpm_fw_upgrade_with_fs(ImageBuffer,fullFileLength,true) == 0)
			...
			else
				{
					...
					if(LCM_ID == 1){
						...
						private_ts->vendor_fw_ver_H = i_CTPM_FW2[FW_VER_MAJ_FLASH_ADDR];
						private_ts->vendor_fw_ver_L = i_CTPM_FW2[FW_VER_MIN_FLASH_ADDR];
						private_ts->vendor_config_ver = i_CTPM_FW2[FW_CFG_VER_FLASH_ADDR];
					}else{
...
static int himax_loadSensorConfig(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	...
	pdata = client->dev.platform_data;
		...
		if (config_selected) {
			//Read config ID
			I("%s : config_selected i2c_himax_master_write\n",__func__);
			private_ts->vendor_config_ver = config_selected->c40[1];

			//Normal Register c1~c35
			//ret = i2c_himax_master_write(client, config_selected->c1,sizeof(config_selected->c1),DEFAULT_RETRY_CNT);
			//I("%s : ret = %d \n",__func__,ret);
...
#ifdef HX_TOUCH_REC
	checktouch = 3;
	/* check data which is like MXPT,RX NUM,TX NUM and so on...if it's need torecover */
	if (check_touchData() == 1)
...
if ((diag_cmd) || (HW_RESET_ACTIVATE)) {
	...
	{
		E("%s: can't read data from chip!\n", __func__);
		if(&ts->suspend_mode){ //for 20160122
		}else{
			...
			if(vk_press == 0x00)
				{
					...
					for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
						if (((finger_pressed >> loop_i) & 1) == 1) {
							...
							input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y); //20160112
		...
		if(HX_INT_IS_EDGE)
			{
				I("%s edge triiger falling\n ",__func__);
#ifdef CONFIG_OF_TOUCH
				ret = request_irq(client->irq, tpd_eint_interrupt_handler, EINTF_TRIGGER_FALLING, "TOUCH_PANEL-eint", NULL);
				if(ret > 0){
				    ret = -1;
...
//CEI factory Start
/*
static ssize_t himax_fw_version_read(struct file *file, char *buf,
		size_t len, loff_t *pos)
{
	size_t ret = 0;
	char buff[80] = {0};
	buff[0]='v';
	debug_level_cmd = buff[0];
		...
		ret += sprintf(buf + ret, "%2.2X%2.2X%2.2X\n",
				private_ts->vendor_fw_ver_H,private_ts->vendor_fw_ver_L,private_ts->vendor_config_ver);
		HX_PROC_SEND_FLAG=1;
	}else{
		HX_PROC_SEND_FLAG=0;
...
static ssize_t himax_fw_version_write(struct file *file, const char *buff,
		        size_t len, loff_t *pos)
{
	return len;
}

static struct file_operations himax_proc_fw_version_ops =
{
	.owner = THIS_MODULE,
	.read = himax_fw_version_read,
	.write = himax_fw_version_write,
};
*/
...
static const struct file_operations himax_proc_fw_version_ops = {
	.owner = THIS_MODULE,
...
static ssize_t himax_check_fw_version_read(struct file *file, char *buf,
		                size_t len, loff_t *pos)
{
	        size_t ret = 0;
		char buff[80] = {0};
		...
		if(!HX_PROC_SEND_FLAG){
			...
			D("%s: %2.2x%2.2x%2.2x\n",__func__,
				private_ts->vendor_fw_ver_H,private_ts->vendor_fw_ver_L,private_ts->vendor_config_ver);
			if(LCM_ID == 1){
				CFV = (( private_ts->vendor_fw_ver_H == i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] )
						&& ( private_ts->vendor_fw_ver_L == i_CTPM_FW[FW_VER_MIN_FLASH_ADDR] )
						&& ( private_ts->vendor_config_ver == i_CTPM_FW[FW_CFG_VER_FLASH_ADDR] ));
				D("O-film\n");
			}else if(LCM_ID == 2){
			...
			if (CFV){
			ret += sprintf(buf + ret, "PASS, Version : %2.2X%2.2X%2.2x\n",
					private_ts->vendor_fw_ver_H,private_ts->vendor_fw_ver_L,private_ts->vendor_config_ver);
			}else{
			ret += sprintf(buf + ret, "FAIL, Version : %2.2X%2.2X%2.2x\n",
					private_ts->vendor_fw_ver_H,private_ts->vendor_fw_ver_L,private_ts->vendor_config_ver);
...
static ssize_t himax_check_fw_version_write(struct file *file, const char *buff,
		                        size_t len, loff_t *pos)
{
	return len;
}

static struct file_operations himax_proc_check_fw_version_ops =
{
	.owner = THIS_MODULE,
	.read = himax_check_fw_version_read,
	.write = himax_check_fw_version_write,
};

static ssize_t himax_enable_tp_read(struct file *file, char *buf,
		                size_t len, loff_t *pos)
{
	        size_t ret = 0;
		once_run = 1 ;
		if(once_run){
			D("%s\n",__func__);
			himax_int_enable(private_ts->client->irq,1,true);
			himax_HW_reset(true,true);  //20160112 to fix power off version
			mdelay(150);  //20160112 to fix power off version 
			once_run = 0;
			D("%s = irq_enable_count = %d\n", __func__, irq_enable_count);
		}
		return ret;
}

static ssize_t himax_enable_tp_write(struct file *file, const char *buff,
		                        size_t len, loff_t *pos)
{
	return len;
}

static struct file_operations himax_proc_enable_tp_ops =
{
	.owner = THIS_MODULE,
	.read = himax_enable_tp_read,
	.write = himax_enable_tp_write,
};

static ssize_t himax_disable_tp_read(struct file *file, char *buf,
		                size_t len, loff_t *pos)
{
		size_t ret = 0;
...
static ssize_t himax_disable_tp_write(struct file *file, const char *buff,
		                        size_t len, loff_t *pos)
{
	return len;
}

static struct file_operations himax_proc_disable_tp_ops =
{
	.owner = THIS_MODULE,
	.read = himax_disable_tp_read,
	.write = himax_disable_tp_write,
};

static ssize_t himax_touch_tp_ic_read(struct file *file, char *buf,
		                size_t len, loff_t *pos)
{
	        size_t ret = 0;
		...
		if(!HX_PROC_SEND_FLAG){
			...
			if(private_ts->vendor_sensor_id){
				ret += sprintf(buf + ret, "PASS, sensor_id: %2.2x\n",
					private_ts->vendor_sensor_id);
			}else{
				ret += sprintf(buf + ret, "FAIL, sensor_id: %2.2x\n",
					private_ts->vendor_sensor_id);
			}
...
static ssize_t himax_touch_tp_ic_write(struct file *file, const char *buff,
		                        size_t len, loff_t *pos)
{
	return len;
}

static struct file_operations himax_proc_touch_tp_ic_ops =
{
	.owner = THIS_MODULE,
	.read = himax_touch_tp_ic_read,
	.write = himax_touch_tp_ic_write,
};
//CEI factory End
...
#ifdef HX_TP_PROC_RESET
//#ifdef TP_ENG_NODE
//	himax_proc_reset_file = proc_create(HIMAX_PROC_RESET_FILE, (S_IRWXU | S_IRWXG | S_IRWXO),
...
#ifdef HX_TP_PROC_DIAG
//#ifdef TP_ENG_NODE
//	himax_proc_diag_file = proc_create(HIMAX_PROC_DIAG_FILE, (S_IRWXU | S_IRWXG | S_IRWXO),
...
#ifdef HX_TP_PROC_REGISTER
//#ifdef TP_ENG_NODE
//	himax_proc_register_file = proc_create(HIMAX_PROC_REGISTER_FILE, (S_IRWXU | S_IRWXG | S_IRWXO),
...
#endif
//CEI factory Start
	himax_proc_fw_version = proc_create(HIMAX_PROC_FW_VERSION, (S_IWUSR|S_IRUGO),
			himax_touch_proc_dir, &himax_proc_fw_version_ops);
	if (himax_proc_fw_version == NULL)
	{
		E(" %s: proc fw_version file create failed!\n", __func__);
		goto fail_15;
	}

	himax_proc_check_fw_version = proc_create(HIMAX_PROC_CHECK_FW_VERSION, (S_IWUSR|S_IRUGO),
			himax_touch_proc_dir, &himax_proc_check_fw_version_ops);
	if (himax_proc_check_fw_version == NULL)
	{
		E(" %s: proc check_fw_version file create failed!\n", __func__);
		goto fail_16;
	}

	himax_proc_enable_tp = proc_create(HIMAX_PROC_ENABLE_TP, (S_IWUSR|S_IRUGO),
			himax_touch_proc_dir, &himax_proc_enable_tp_ops);
	if (himax_proc_enable_tp == NULL)
	{
		E(" %s: proc enable_tp file create failed!\n", __func__);
		goto fail_17;
	}

	himax_proc_disable_tp = proc_create(HIMAX_PROC_DISABLE_TP, (S_IWUSR|S_IRUGO),
			himax_touch_proc_dir, &himax_proc_disable_tp_ops);
	if (himax_proc_disable_tp == NULL)
	{
		E(" %s: proc disalbe_tp file create failed!\n", __func__);
		goto fail_18;
	}

	himax_proc_touch_tp_ic = proc_create(HIMAX_PROC_TOUCH_TP_IC, (S_IWUSR|S_IRUGO),
			himax_touch_proc_dir, &himax_proc_touch_tp_ic_ops);
	if (himax_proc_touch_tp_ic == NULL)
	{
		E(" %s: proc touch_tp_ic file create failed!\n", __func__);
		goto fail_19;
	}
...
//CEI factory Start
	fail_19:
	remove_proc_entry( HIMAX_PROC_TOUCH_TP_IC, himax_touch_proc_dir);
	fail_18:
	remove_proc_entry( HIMAX_PROC_DISABLE_TP, himax_touch_proc_dir);
	fail_17:
	remove_proc_entry( HIMAX_PROC_ENABLE_TP, himax_touch_proc_dir);
	fail_16:
	remove_proc_entry( HIMAX_PROC_CHECK_FW_VERSION, himax_touch_proc_dir);
	fail_15:
	remove_proc_entry( HIMAX_PROC_FW_VERSION, himax_touch_proc_dir);
//CEI factory End
#ifdef HX_SMART_WAKEUP
	fail_14:
#endif
...
/*	struct regulator *vdd; //20160118 */

	u32 data = 0;
	
	vdd = regulator_get(ts->dev,"avdd");
#ifndef REGULATOR_AUTO_FOR_SYSTEM
	rc = regulator_enable(vdd);
	D("%s:vdd rc = %d\n",__func__,rc);
#endif
	rc = regulator_is_enabled(vdd);
	D("%s: regulator is_enable %d\n", __func__, rc);
...
himax_gpio_power_config(ts->client, pdata);
	...
	LCM_ID = simple_strtol(android_boot_dev_display ,&end_ptr ,10);
	D("LCM_ID = %ld\n",LCM_ID);
...
static int himax852xes_suspend(struct device *dev)
{
	int ret;
	int rc;
	uint8_t buf[2] = {0}; //20160112 to fix power off version
	//int rc;
#ifdef HX_CHIP_STATUS_MONITOR
	int t=0;
#endif
...
#endif
	//mdelay(30);
///*  //20160112 to fix power off version
...
static int himax852xes_resume(struct device *dev)
{
	...
	I("%s: enter \n", __func__);
	ts = dev_get_drvdata(dev);
	if (atomic_read(&ts->resumed)) {
		I("%s: Already resume Skipped. \n", __func__);
		return 0;
	}
	atomic_set(&ts->resumed, 1);
	...
	if (rc) {
		...
		if (rc)
			D("%s: retry to Success. regulator_is_enable = %d\n", __func__, rc);
		else
			D("%s: Fail. regulator_is_enable = %d\n", __func__, rc);
	}
	atomic_set(&ts->suspend_mode, 0);

	/*20161101 TP reset,if MCU status not Running*/

/*	MCU_status = himax_hand_shaking();

	if (MCU_status != 0) {
		D("%s Loren test himax_hand_shaking MCU status = %d\n", __func__, MCU_status);
		himax_HW_reset(true, true);
	}
*/
	himax_int_enable(ts->client->irq,1,true);
...
#ifdef MTK
static struct i2c_board_info __initdata himax852xes_i2c = { I2C_BOARD_INFO(HIMAX852xes_NAME, (0x94 >> 1))};
