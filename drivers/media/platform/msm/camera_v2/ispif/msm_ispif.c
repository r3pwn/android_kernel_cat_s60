...
#ifdef ORG
#else
static int cam_isp_status_read(struct seq_file *m, void *v)
{
	char status[16];

	pr_err("cam_isp_status_read\n");

	switch (gnCamISPStatus) {
	case 1:
	...
	seq_printf(m, "%s\n", status);
	pr_err("Got Cam ISP Status = %s\n", status);
	return 0;
}

static int cam_isp_status_open(struct inode *inode, struct file *file)
{
	return single_open(file, cam_isp_status_read, NULL);
}

static const struct file_operations cam_isp_status_fops = {
	.open = cam_isp_status_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int ispif_probe(struct platform_device *pdev)
{
	...
	ispif->open_cnt = 0;
	/*B, for ERB*/
	proc_create("cei_cam_isp_status", S_IRUSR | S_IRGRP | S_IROTH, NULL, &cam_isp_status_fops);
	/*E, for ERB*/
	return 0;
