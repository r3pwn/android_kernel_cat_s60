...
int gnCam0I2cStatus = 0, gnCam1I2cStatus = 0;
#endif
/*E, for ERB*/
	...
	struct msm_sensor_ctrl_t  *s_ctrl)
{
	...
	if (s_ctrl) {
		...
		if (rc < 0) {
			...
			return snprintf(buf, PAGE_SIZE, "ID: error 0x03\n");
		}

		CDBG("Start read id, Addr[%d]\n", s_ctrl->sensordata->slave_info->sensor_id_reg_addr);

		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client,
				s_ctrl->sensordata->slave_info->sensor_id_reg_addr, &nChipId, MSM_CAMERA_I2C_WORD_DATA);

		s_ctrl->func_tbl->sensor_power_down(s_ctrl);
		if (rc < 0) {
			pr_err("Read id failed!\n");
			return snprintf(buf, PAGE_SIZE, "ID: error 0x04\n");
		}

		CDBG("Check id okay\n");
	} else {
		pr_err("s_ctrl get fail!\n");
		return snprintf(buf, PAGE_SIZE, "ID: error 0x02\n");
	}

	CDBG("Exit Ping function\n");
	return snprintf(buf, PAGE_SIZE, "ID: 0x%x\n", nChipId);
}

ssize_t CheckCameraID_main_show(struct file *f, char __user *data,
					size_t count, loff_t *ppos)
{
	...
	s_ctrl = g_sctrl[0];
	res = CheckCameraID(buf, s_ctrl);
	return simple_read_from_buffer(data, count, ppos, buf, res);
}
static const struct file_operations CheckCameraID_main_ops = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.read = CheckCameraID_main_show
};

ssize_t CheckCameraID_sub_show(struct file *f, char __user *data,
					size_t count, loff_t *ppos)
{
	char buf[128];
	int res;
...
static const struct file_operations CheckCameraID_sub_fops = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.read = CheckCameraID_sub_show
};

/*B, for ERB*/
#ifdef ORG
#else
static int cam0_i2c_status_read(struct seq_file *m, void *v)
{
	char status[16];

	pr_err("cam0_i2c_status_read\n");

	if (1 == gnCam0I2cStatus)
		strlcpy(status, "success", 8);
	else if (2 == gnCam0I2cStatus)
		strlcpy(status, "failed", 7);
	else
		strlcpy(status, "unknown", 8);

	seq_printf(m, "%s\n", status);
	pr_err("Got Cam0 I2C Status = %s\n", status);
	return 0;
}

static int cam1_i2c_status_read(struct seq_file *m, void *v)
{
	char status[16];

	pr_err("cam1_i2c_status_read\n");

	if (1 == gnCam1I2cStatus)
		strlcpy(status, "success" , 8);
	else if (2 == gnCam1I2cStatus)
		strlcpy(status, "failed", 7);
	else
		strlcpy(status, "unknown", 8);

	seq_printf(m, "%s\n", status);
	pr_err("Got Cam1 I2C Status = %s\n", status);
	return 0;
}

static int cam0_i2c_status_open(struct inode *inode, struct file *file)
{
	return single_open(file, cam0_i2c_status_read, NULL);
}

static int cam1_i2c_status_open(struct inode *inode, struct file *file)
{
	return single_open(file, cam1_i2c_status_read, NULL);
}

static const struct file_operations cam0_i2c_status_fops = {
	.open = cam0_i2c_status_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static const struct file_operations cam1_i2c_status_fops = {
	.open = cam1_i2c_status_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
CSID_TG:
	...
	*  sensor control structure */
	...
	if (slave_info->camera_id == CAMERA_0)
		proc_create("cei_cam0_i2c_status", S_IRUSR | S_IRGRP | S_IROTH, NULL, &cam0_i2c_status_fops);
	else if (slave_info->camera_id == CAMERA_1)
		proc_create("cei_cam1_i2c_status", S_IRUSR | S_IRGRP | S_IROTH, NULL, &cam1_i2c_status_fops);
	/*E, for ERB*/
	...
	debugfs_create_file("CheckCameraID_main", 0400, NULL, NULL, &CheckCameraID_main_ops);
	debugfs_create_file("CheckCameraID_sub", 0400, NULL, NULL, &CheckCameraID_sub_fops);
