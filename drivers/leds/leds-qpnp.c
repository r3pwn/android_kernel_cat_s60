...
static int qpnp_mpp_set(struct qpnp_led_data *led)
{
	...
	if (led->cdev.brightness) {
		...
		if (led->mpp_cfg->pwm_mode == PWM_MODE) {
			...
			if (!strcmp(led->cdev.name, "blue") && led->cdev.brightness == 255)
				period_us = 10000;
			else
				...
				enum led_brightness value)
{
	...
	dev_err(&led->spmi_dev->dev, "qpnp_led_set brightness=%d\n", value);
...
static void set_pwm_led_reg(struct qpnp_led_data *led)
{
	u16 addr;
	u8 reg;
	int rc;

	dev_err(&led->spmi_dev->dev, "[set_pwm_led_reg] name=%s\n", led->cdev.name);
	if (first_set)
		return;

	addr = 0xD8d0;
	reg = 0xa5;
	rc = spmi_ext_register_writel(led->spmi_dev->ctrl, 3,
			addr, &reg, 1);
	if (rc)
		dev_err(&led->spmi_dev->dev,
			"Unable to write to addr=%x, rc(%d)\n", addr, rc);
				...
				enum led_brightness value)
{
	...
	case QPNP_ID_LED_MPP:
		if (!strcmp(led->cdev.name, "blue")) {
			if (value > 0 && value < 255)
				led->cdev.brightness = 255;
		}

		if (!strcmp(led->cdev.name, "green") && value == 66)
		...
		if (!strcmp(led->cdev.name, "green")) {
			if ((value == 128) || (value == 80))
				smbchg_chg_rled_brightness_set(tchip, 255);
			else if (value == 66) {
				dev_err(&led->spmi_dev->dev,
					"set red led (0)\n");
				smbchg_chg_rled_brightness_set(tchip, 0);
			}
		}
		break;
...
static void MPP_LED_blink(struct qpnp_led_data *led, int blinking)
{
	int rc;

	dev_err(&led->spmi_dev->dev, "MPP_LED_blink name=%s blinking=%d\n", led->cdev.name, blinking);

	set_pwm_led_reg(led);

	if (blinking)
		led->cdev.brightness = 23; /*500ms on; 5s off*/
	else
		led->cdev.brightness = 0;
	...
	rc = qpnp_mpp_set(led);
	if (rc < 0)
		dev_err(&led->spmi_dev->dev, "MPP_LED_blink set brightness failed (%d)\n", rc);
	mutex_unlock(&led->lock);
}
				...
				struct device_node *node, const char *name)
{
	...
	pwm_cfg->pwm_dev = of_pwm_get(node, name);
				...
				struct device_node *node)
{
	...
	rc = qpnp_get_config_pwm(led->kpdbl_cfg->pwm_cfg, led->spmi_dev,  node, led->cdev.name);
	if (rc < 0)
		return rc;
				...
				struct device_node *node)
{
	...
	rc = qpnp_get_config_pwm(led->rgb_cfg->pwm_cfg, led->spmi_dev, node, led->cdev.name);
	if (rc < 0)
		return rc;
		...
		struct device_node *node)
{
	...
	if (!rc) {
		...
		if (led_mode == MANUAL_MODE && strcmp(led->cdev.name, "green"))
			return MANUAL_MODE;
	...
	rc = qpnp_get_config_pwm(led->mpp_cfg->pwm_cfg, led->spmi_dev, node, led->cdev.name);
	if (rc < 0)
		goto err_config_mpp;
	if (!strcmp(led->cdev.name, "green"))
		return MANUAL_MODE;
...
static int qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	for_each_child_of_node(node, temp) {
		...
		dev_err(&spmi->dev, "led->id=%d name=%s\n", led->id, led->cdev.name);
